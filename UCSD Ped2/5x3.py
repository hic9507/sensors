...
args = {"data_folder": "D:\\InChang\\abnormal_detection_dataset\\UCSD_Anomaly_Dataset.v1p2\\UCSDped2\\",
        "graphs_folder": "./graph/5x3/resnet10/", "epoch": 100, "batch_size": 1, "num_class": 1, "learning_rate": 5e-4,
        "decay_rate": 0.0001, "num_workers": 4, "img_size": (640, 360), "img_depth": 3, "FPS": 22,
        "SEED": 42}
...
model_save_folder = './trained_model/5x3/'
...
class CustomDataset(Dataset):
    def __init__(self, root_dir, transform=None, FPS=args["FPS"]):
    ...
    def __getitem__(self, index):
        ...
        for i in range(1, min(201, total_files) + 1):  # 변경된 부분
        ...
        if os.path.exists(gt_path):
            ...
            for i in range(1, min(201, total_lbl_files) + 1):  # 변경된 부분
            ...
            label_tensor = torch.zeros_like(video_tensor)
...
train_dataset = CustomDataset(root_dir=args["data_folder"] + "/Test-copy/")
val_dataset = CustomDataset(root_dir=args["data_folder"] + "/Train")
...
def train(model, device, args, train_loader, val_loader):
    ...
    for epoch in range(1, args['epoch'] + 1):
        ...
        for videos, labels in tqdm(train_loader, total=len(train_loader), desc='Train', ascii='->=', leave=False):
            ...
            output_expanded = output.repeat(args["batch_size"], 1, 22, 400, 400)

            # 확장된 출력을 [batch_size, 1]로 변환
            output_avg = output_expanded.mean(dim=[2, 3, 4])

            # 이진 분류를 위한 라벨 생성
            binary_label_value = 1.0 if (labels == 1).any() else 0.0
            binary_labels_ = torch.tensor([[binary_label_value]]).to(device)

            train_loss = criterion(output_avg, binary_labels_)

            outputs = (torch.sigmoid(output_avg) > 0.1).float()  # 임계값을 0.5로 설정하여 예측값 변환
            ...
            y_true.append(binary_labels_.detach().cpu().numpy())
        ...
        with torch.no_grad():
            ...
            for videos, labels in tqdm(val_loader, total=len(val_loader), desc='Validation', ascii='->=', leave=False):
                ...
                test_loss = criterion(logit_avg, binary_labels_)

                logits = (torch.sigmoid(logit_avg) > 0.5).float()  # 임계값을 0.5로 설정하여 예측값 변환
