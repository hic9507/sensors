...
args = {"data_folder": "D:\\abnormal_detection_dataset\\UBI_FIGHTS\\videos\\All\\",
        "graphs_folder": "./graph/4x4_same/resnet10/", "epoch": 150, "batch_size": 1, "num_class": 1, "learning_rate": 1e-4,
...
model_save_folder = './trained_model/4x4_same/'
...
train_normal_dir = 'D:/InChang/abnormal_detection_dataset/UBI-FIGHTS/train_normal_same/normal_same/'
val_fight_dir = 'D:/InChang/abnormal_detection_dataset/UBI-FIGHTS/val/fight/'
val_normal_dir = 'D:/InChang/abnormal_detection_dataset/UBI-FIGHTS/val_normal_same/normal_same/'
annotation_dir = 'D:/InChang/abnormal_detection_dataset/UBI-FIGHTS/annotation/'


class CustomDataset(Dataset):
    def __init__(self, video_files, annotation_files, transform=None, FPS=args["FPS"], img_size=args["img_size"], frame_chunk=16):
    ...
    def get_video_and_label(self, idx):
        ...
        fight_indices = np.where(annotation_df.values.squeeze() == 1)[0][::5]           # 1, 6, 7처럼 5프레임 간격으로 프레임 인덱스를 읽어옴
        ...
        for chunk_start in range(0, len(indices), self.frame_chunk):
            ...
            for i in chunk_indices:
                ...
                frames.append(frame)
                labels.append(annotation_df.iloc[i].values[0])          #     1이나 0만 추가. labels.append*annotation_df.iloc[i][0])도 가능
...
# 각 클래스의 결과를 합쳐 전체 train, val, test 파일과 주석을 얻습니다.
...
class EarlyStopping:
    def __init__(self, patience=15, verbose=False, delta=0, path=f"{model_save_folder}" + f"model_depth{model_depth}" + f"_early.pth"):
...
def train(model, device, args, train_loader, val_loader):
    ...
    optimizer = torch.optim.AdamW(model.parameters(), lr=args["learning_rate"], weight_decay=args["decay_rate"], amsgrad=True)  # .0004
